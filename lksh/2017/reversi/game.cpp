...
float calc(const vector<vector<int>>& fld, const vector<vector<int>>& old_fld, int c){
    float ans = (fld[0][0] == c) * 10.0;
    ans += (fld[0][7] == c) * 10.0;
    ans += (fld[7][0] == c) * 10.0;
    ans += (fld[7][7] == c) * 10.0;
    for (int i = 0; i < 8;i++){
...
float dfs(const vector<vector<int>>& field, int is_mx, float alpha, float beta, int depth, int c,const vector<vector<int>>& old_fld, int original){
...
void move(int a[8][8], const int c, int* row, int* column){
    ...
    assert(!turns.empty());
    float value = -FLT_MAX, alpha = -FLT_MAX, beta = FLT_MAX;
    Turn max_ind = {{}, 0, 0, 0};
    for (Turn turn : turns){

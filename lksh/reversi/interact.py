...
def main():
    ...
    check(len(sys.argv) == 3, "usage: " +
          sys.argv[0] + " [--verbose] player1 player2")

    pp = [None, None]

    if sys.argv[1] != "":
        pp[0] = subprocess.Popen(
            sys.argv[1], stdin=subprocess.PIPE, stdout=subprocess.PIPE, universal_newlines=True)

    if sys.argv[2] != "":
        pp[1] = subprocess.Popen(
            sys.argv[2], stdin=subprocess.PIPE, stdout=subprocess.PIPE, universal_newlines=True)
    ...
    if not game.fail():
        a, b = game.score()
        msg = "draw" if a == b else (
            (NN[BLACK] if a < b else NN[WHITE]) + " wins")
        print("")

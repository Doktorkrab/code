...
int get(int pos, int tl, int tr, int l, int r, int x){
    ...
    // cerr << ' ' << (int)(lower_bound(t[pos].sorted.begin(), t[pos].sorted.end(), x) - t[pos].sorted.begin()) << '\n';
    if (l <= tl && tr <= r) return (int)(lower_bound(t[pos].sorted.begin(), t[pos].sorted.end(), x) - t[pos].sorted.begin());
    int tm = (tl + tr) / 2;

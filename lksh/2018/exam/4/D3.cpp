...
int main(){
    SuffixArray<MAXN> suf;
    string s;
    cin >> s;
    suf.BuildArray(s.size(), s.c_str());
    suf.BuildLCP(s.size(), s.c_str());
    int64_t k;
    cin >> k;
    --k;
    suf.len[0] = 0;
    for (int i = 0; i < s.size(); i++){
        cerr << suf.len[i] << '\n';
        cerr << suf.p[i] << ' ' << s.size() - suf.p[i] - suf.len[i] << ' ' << k << '\n';
        if (s.size() - suf.p[i] - suf.len[i] > k){
            cerr << suf.p[i] << '\n';
            for (int j = 0; j < k; j++){
                cout << s[suf.p[i] + j];
            }

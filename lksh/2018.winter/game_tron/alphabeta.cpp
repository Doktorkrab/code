...
//    return -k * (sum - sum1) + (1 - k) * (top.first - top.second);
    return top.second * sum - top.first * sum1;
}

double minimax(int depth, vector<vector<int>> field, int player, double alpha, double beta) {
    if (depth == MAX_DEPTH) {
    ...
    if (player == 1) {
        ...
        for (Position &pos : deltas) {
            ...
            auto value = minimax(depth + 1, field, 3 - player, alpha, beta);
...
Position calcNewTurn() {
    ...
    for (Position pos : deltas) {
        ...
        double tmpScore = minimax(0, preparedField, 2, -DBL_MAX, DBL_MAX);
